...
colors:
  ...
  pos: "oklch(0.455 0.090 45)"
  neu: "oklch(0.700 0.006 80)"
  neg: "oklch(0.480 0.090 250)"
...
### Tertiary
- **暖墨 / 中性 / 冷墨**（`pos` / `neu` / `neg`）：情感兩極。一暖一冷的墨，不是紅綠；朱批與藍鉛筆是編輯的手，不下放做資料色。彩度 0.090 —— 要讀者分得出暖冷（更淡就只是「帶點顏色的黑」），又明顯低於朱批的 0.190，不與編輯的手爭。暖墨紙上 6.6:1、冷墨 5.7:1，兩者都能當文字用；中性墨只做色塊，永遠不當文字。
...
**鉛字級數 The Type-Size-Is-Data Rule.** 詞頻用字級表達（`WordTable` 七階：12 / 13 / 15 / 17 / 19 / 21 / 24px），像報紙用字級排新聞輕重。不做詞雲：這是真文字，可讀、可選取、可鍵盤操作，也不吃 canvas。字級是排序的**表徵**，不是版面的形狀 —— 詞排成有欄線的**列表**（序號、詞、次數各佔一欄，容器查詢決定 2／3／4 欄），不是隨行寬換行的散塊：散塊行高忽高忽低、尾巴會落單，而且只有最大的幾個詞印得出次數。
...
### 印出來的圖（signature）
**兩份詞表不排同一件事 The Two Word Tables Rule.** 「詞表」按**次數**排（誰被說得最多，可切換六個來源）；「情感」按**情感分**排，只取兩端 ——「說得最暖的詞」與「說得最冷的詞」，各自印出次數與 −1…1 的分數。同一批詞用同一種排序印兩次、只靠墨色分辨，讀者看到的就是同一張表兩遍；情感那一份必須回答只有情感能回答的問題。方向由欄位與正負號先講清楚，墨色只是附和。
